...
async def process_batch():
    # Use command-line argument for cities file
    cities_file = args.cities or 'cities_top200.txt'
    ...
    logging.info(f"Processing {len(cities)} cities from file {cities_file}")
...
async def main():
    ...
    print(f"Cities file: {args.cities or 'cities_top200.txt'}")
    print(f"{'='*80}\n")
    
    results = await process_batch()
    
    if results:
        # Save results with a standard filename for the workflow
        ...
        print(f"\n{'='*80}")
        print(f"SCRAPING COMPLETED - SUMMARY REPORT")
        print(f"{'='*80}")
        ...
        print(f"\n⚠️  No 10,000-point hotels found")
